...
eval_tyAbs = syndefM eval p_TyAbs (univTy <$> ter ch_tyAbsTy <*> at ch_tyAbsTerm eval <*> at lhs ctx_var)
  where univTy tyAbsTy tyAbsTerm ctx_Var = if Data.Set.member tyAbsTy ctx_Var then UnivTy tyAbsTy tyAbsTerm else error $ "La variable de tipo " ++ show tyAbsTy ++ " no se encuentra en el contexto"
...
ctx_appTyArgTy = inhdefM ctx p_AppTy ch_appArgTy (at lhs ctx)
ctx_appTyResTy = inhdefM ctx p_AppTy ch_appResTy (at lhs ctx)
ctx_univTyTy   = inhdefM ctx p_UnivTy ch_univTyTy (at lhs ctx)
asp_ctx = ctx_abs .+: ctx_appFun .+: ctx_appArg .+: ctx_tyAbs .+: ctx_tyApp .+:
  ctx_appTyArgTy .+: ctx_appTyResTy .+: ctx_univTyTy .+: emptyAspect
...
ctx_var_tyAbs  = inhdefM ctx_var p_TyAbs ch_tyAbsTerm (Data.Set.insert <$> ter ch_tyAbsTy <*> at lhs ctx_var)
ctx_var_tyApp  = inhdefM ctx_var p_TyApp ch_tyAppTerm (at lhs ctx_var)

ctx_var_appTyArgTy = inhdefM ctx_var p_AppTy ch_appArgTy (at lhs ctx_var)
ctx_var_appTyResTy = inhdefM ctx_var p_AppTy ch_appResTy (at lhs ctx_var)
ctx_var_univTyTy   = inhdefM ctx_var p_UnivTy ch_univTyTy (at lhs ctx_var)
asp_ctx_var = ctx_var_abs .+: ctx_var_appFun .+: ctx_var_appArg .+: ctx_var_tyAbs .+: ctx_var_tyApp .+:
  ctx_var_appTyArgTy .+: ctx_var_appTyResTy .+: ctx_var_univTyTy .+: emptyAspect

asp_all = asp_eval .:+: asp_ctx .:+: asp_ctx_var

$(mkSemFunc ''Nt_Ty)
$(mkSemFunc ''Nt_Term)

evalTerm term context = sem_Term asp_all term ( (ctx =. context) .*. (ctx_var =. (Data.Set.empty::Set TypeVariable)) .*. emptyAtt ) #. eval
